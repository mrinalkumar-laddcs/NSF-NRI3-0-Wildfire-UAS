function [NodePath, cost] = dijkstra(s,t,edgeWeight,source,target)
...
%--------%
mesh_graph = digraph(s,t,edgeWeight);
[NodePath,cost,~] = shortestpath(mesh_graph,source,target);
NodePath = NodePath';
return
%--------%
...
if length(s) == length(t) && length(s) == length(edgeWeight)
    ...
    while ~isVisited(tNodeIndex)
        ...
        tEdgeIndices = find(s == nodeID(sNodeIndex));
        tNodeIndices = t(tEdgeIndices,1);
        tCost        = edgeWeight(tEdgeIndices,1);

        if ~isempty(tEdgeIndices)
