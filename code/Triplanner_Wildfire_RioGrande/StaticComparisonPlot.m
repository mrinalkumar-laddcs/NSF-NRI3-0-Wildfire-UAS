...
load(static_conserv_test_file);
stepResult2 = stepResult;
load(static_test_file);
global plot_transform

% colorOpts = {[0 0.4470 0.7410], [0.8500 0.3250 0.0980],[0.4940 0.1840 0.5560],[0.3010 0.7450 0.9330], [0.6350 0.0780 0.1840]};
...
%                 else
%                     patch('Faces',1:1:size(stepResult(i).risk.obs(j).polygon,1),'Vertices',stepResult(i).risk.obs(j).polygon,'LineStyle',':','FaceColor','y','FaceAlpha',0.2);
%                 end
%             end
%         end
%         for i = 1:size(stepResult,2)
%             % show the path
%             pathHandle(i) = plot(stepResult(i).risk.x(:,1),stepResult(i).risk.x(:,2),'--','LineWidth',2,'Color',colorOpts{i});
%             pathText{i}   = ['Planned Traj @ t_' num2str(i)];
...
%             traversedPathHandle(i) = plot(path(:,1),path(:,2),'b-','LineWidth',2,'Color',colorOpts{i});
...
%                 else
%                     patch('Faces',1:1:size(stepResult(i).risk.obs(j).polygon,1),'Vertices',stepResult(i).risk.obs(j).polygon+[domain.longmin domain.latmin],'LineStyle',':','FaceColor','y','FaceAlpha',0.2);
%                 end
%             end
%         end
%         for i = 1:size(stepResult,2)
%             % show the path
%             pathHandle(i) = plot(stepResult(i).risk.x(:,1)+domain.longmin,stepResult(i).risk.x(:,2)+domain.latmin,'--','LineWidth',2,'Color',colorOpts{i});
%             pathText{i}   = ['Planned Traj @ t_' num2str(i)];
%             plot(stepResult(i).risk.x(end,1)+domain.longmin,stepResult(i).risk.x(end,2)+domain.latmin,'ro','LineWidth',1);
%             path = interp1(stepResult(i).risk.t,stepResult(i).risk.x,0:0.5:approach.rolling.tDelta);
%             traversedPathHandle(i) = plot(path(:,1)+domain.longmin,path(:,2)+domain.latmin,'b-','LineWidth',2,'Color',colorOpts{i});
%             traversedPathText{i}   = ['Traversed Traj @ t_' num2str(i) '-t_' num2str(i+1)];
%         end
%         % show the start/end points
...
%     legend([pathHandle(1:end) pStart traversedPathHandle(1:end) pFinal],{pathText{:},'Start Point',traversedPathText{:},'Final Point'},'Location','north','NumColumns',2);
    ...
    if plot_transform == 1
        % show the obstacles
        for j = 1:size(stepResult2.risk.obs,2)
            patch('Faces',1:1:size(stepResult2.risk.obs(j).polygon,1),'Vertices',stepResult2.risk.obs(j).polygon,'LineStyle',':','FaceColor','y','FaceAlpha',0.2);
...
%             plot(stepResult.risk.obs(j).polygon(:,1),stepResult.risk.obs(j).polygon(:,2), 'k-','LineWidth',1);
        ...
        % show the path
        pathHandle_con = plot(stepResult2.risk.x(:,1),stepResult2.risk.x(:,2),'m:','LineWidth',2);
        
        % show the obstacles
        for j = 1:size(stepResult.risk.obs,2)
            patch('Faces',1:1:size(stepResult.risk.obs(j).polygon,1),'Vertices',stepResult.risk.obs(j).polygon,'LineStyle','-','FaceColor','y','FaceAlpha',1);
%             fill(stepResult.risk.obs(j).polygon(:,1),stepResult.risk.obs(j).polygon(:,2), 'y');
%             plot(stepResult.risk.obs(j).polygon(:,1),stepResult.risk.obs(j).polygon(:,2), 'k-','LineWidth',1);
        end
        % show the path
        ...
        % show the obstacles
        for j = 1:size(stepResult2.risk.obs,2)
            patch('Faces',1:1:size(stepResult2.risk.obs(j).polygon,1),'Vertices',stepResult2.risk.obs(j).polygon+[domain.longmin domain.latmin],'LineStyle','--','FaceColor','y','FaceAlpha',0.2);
...
%             plot(stepResult.risk.obs(j).polygon(:,1)+domain.longmin,stepResult.risk.obs(j).polygon(:,2)+domain.latmin, 'k-','LineWidth',1);
        ...
        % show the path
        pathHandle_con = plot(stepResult2.risk.x(:,1)+domain.longmin,stepResult2.risk.x(:,2)+domain.latmin,'m:','LineWidth',2);
        
        % show the obstacles
        for j = 1:size(stepResult.risk.obs,2)
            patch('Faces',1:1:size(stepResult.risk.obs(j).polygon,1),'Vertices',stepResult.risk.obs(j).polygon+[domain.longmin domain.latmin],'LineStyle','-','FaceColor','y','FaceAlpha',1);
%             fill(stepResult.risk.obs(j).polygon(:,1)+domain.longmin,stepResult.risk.obs(j).polygon(:,2)+domain.latmin, 'y');
%             plot(stepResult.risk.obs(j).polygon(:,1)+domain.longmin,stepResult.risk.obs(j).polygon(:,2)+domain.latmin, 'k-','LineWidth',1);
        end
    ...
    legend([pathHandle pathHandle_con pStart pFinal],{['Planned Traj \epsilon = ' num2str(0.05)],['Planned Traj \epsilon = ' num2str(0)],'Start Point','Final Point'},'Location','north','NumColumns',2);
    
    dist = sum(sqrt((stepResult.risk.x(2:end,1)-stepResult.risk.x(1:end-1,1)).^2 + (stepResult.risk.x(2:end,2)-stepResult.risk.x(1:end-1,2)).^2))
    dist2 = sum(sqrt((stepResult2.risk.x(2:end,1)-stepResult2.risk.x(1:end-1,1)).^2 + (stepResult2.risk.x(2:end,2)-stepResult2.risk.x(1:end-1,2)).^2))
end
...
if approach.typeID == 1
    print(gcf,[rolling_test_file,'_comparison'],'-dpng','-r600');
else
    print(gcf,[static_test_file,'_comparison'],'-dpng','-r600');
end