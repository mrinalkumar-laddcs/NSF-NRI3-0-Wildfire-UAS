...
if strcmp(heat_file,'TaosFire.mat')
%     guessGen.x = guessGen3.x;
%     guessGen.y = guessGen3.y;
    guessGen.x = guessGen6.x;
    guessGen.y = guessGen6.y;
else
    guessGen.x = [X0,10, 10, Xf]';
    guessGen.y = [Y0,(Y0+Yf)/5,4*(Y0+Yf)/5,Yf]';
end
...
if strcmp(theta0,'free')
    bounds.phase.initialstate.lower = [X0, Y0, V0, thetarMin, T0];
    bounds.phase.initialstate.upper = [X0, Y0, V0, thetarMax, T0];
else
    bounds.phase.initialstate.lower = [X0, Y0, V0, thetar0, T0];
    bounds.phase.initialstate.upper = [X0, Y0, V0, thetar0, T0];
end
bounds.phase.state.lower = [xMin, yMin, VMin, thetarMin, TMin];
bounds.phase.state.upper = [xMax, yMax, VMax, thetarMax, TMax];
if strcmp(thetaf,'free')
    bounds.phase.finalstate.lower = [Xf, Yf, Vf, thetarMin, TMin];
    bounds.phase.finalstate.upper = [Xf, Yf, Vf, thetarMax, TMax];
else
    bounds.phase.finalstate.lower = [Xf, Yf, Vf, thetarf, TMax];
    bounds.phase.finalstate.upper = [Xf, Yf, Vf, thetarf, TMax];
end
bounds.phase.control.lower = [AMin, uMin];
bounds.phase.control.upper = [AMax, uMax];
if obsEnable ~= 0
...
delta_t = [diff(t_star)];
dist = sum(V_star(1:end-1,1).*delta_t);
temp = delta_t.*interp2(heatdata.X,heatdata.Y,heatdata.HF,(X_star(1:end-1,1)+X_star(2:end,1))/2,(Y_star(1:end-1,1)+Y_star(2:end,1))/2)*0.08;
E_hover  = Ph*delta_t;
E_motion = Pm*V_star(1:end-1,1).*delta_t;
E = sum(E_hover + E_motion);

t_star  = solution(1).phase.time;
a_star = solution(1).phase.control(:,1);
u_star = solution(1).phase.control(:,2);
X_star = solution(1).phase.state(:,1);
Y_star = solution(1).phase.state(:,2);
V_star = solution(1).phase.state(:,3);
thetar_star = solution(1).phase.state(:,4);
T_star = solution(1).phase.state(:,5);
X_guess  = guess(1).phase.state(:,1);
Y_guess  = guess(1).phase.state(:,2);
...
result.dist = dist;
result.E = E;
save('ComparisonResult_TempCon.mat','result');
...
figure
plot(guess.phase.time,guess.phase.control(:,2));
hold on
plot(t_star,u_star);
...
figure
plot(guess.phase.time,guess.phase.state(1,4)+[0; cumsum((guess.phase.time(2:end,1)-guess.phase.time(1:end-1,1)).*guess.phase.control(2:end,2))]);
hold on
