...
if strcmp(mode,'map')
    ...
    path_noise.sigma_u = 0.2;
    path_noise.delta_t = 0.4;
    
    save_file = [save_file,num2str(polyCase)];
end

if strcmp(unc_mode,'risk')
    delta_vec = round(norminv(1-risk.value,risk.mu, sqrt(risk.sigma^2+(path_noise.sigma_u*V*path_noise.delta_t^1.5)^2)),3);
else
...
% run('twoDoneParticleObstaclePlot_RiskReward_dynanoise');